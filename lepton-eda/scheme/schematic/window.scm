           ...
           (when (in-action? window)
             (if (not (null-pointer? (schematic_window_get_place_list *window)))
                 ...
                 (match current-action-mode
                   ...
                   ('box-select-mode (finish-box-selection window unsnapped-x unsnapped-y))
                       ...
                       (cond
                        ((= middle-button MOUSEBTN_DO_ACTION)
                         ...
                         (unless (true? (schematic_window_get_shift_key_pressed *window))
                           (find-object *window unsnapped-x unsnapped-y))
...
;;; Adds menu widget *MENUBAR to the main window widget
...
--with-gtk3).  TOPLEVEL is a foreign LeptonToplevel structure."
  ...
  (let ((*main-window (schematic_window_create_app_window *app)))
    ...
    (let ((*main-box (schematic_window_create_main_box *main-window))
      ...
      (let ((*translate-widget (schematic_translate_widget_new)))
        (gtk_widget_pack_child *work-box *translate-widget)
      ...
      (let* ((*options (schematic_window_get_options *window))
             ...
             (third-button-cancel-action-text
              (cond
               ((= third-button-cancel) (G_ "/Cancel"))
               (else (= third-button-cancel) "")))
