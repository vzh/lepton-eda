          ...
          if (lepton_object_is_pin (o_lower_current))
          {
            ...
            while (a_iter != NULL) {
              ...
              if (lepton_object_is_text (pin_attrib)
                  && lepton_object_get_text (pin_attrib) != NULL) {  /* found an attribute */
          attrib_text = g_strdup (lepton_text_object_get_string (pin_attrib));
                ...
                if ( (strcmp(attrib_name, "pinnumber") != 0)
                  ...
                  if (row == -1 || col == -1) {
                    ...
                    attrib_table_set_row (pin_table, row, col, row);
                    attrib_table_set_column (pin_table, row, col, col);
                    attrib_table_set_row_name (pin_table, row, col, row_label);
                    attrib_table_set_column_name (pin_table, row, col, attrib_name);
                    attrib_table_set_attrib_value (pin_table, row, col, attrib_value);
                  }
                }
                g_free(attrib_name);
                g_free(attrib_text);
